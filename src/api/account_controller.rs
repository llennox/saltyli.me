use crate::{
    ...
    constants,
    models::{
        user::{LoginDTO, UserInput, User, UserOutput},
...
pub async fn signup(user_dto: web::Json<UserInput>, pool: web::Data<Pool>) -> Result<HttpResponse> {
...
// POST api/auth/me
pub async fn me(req: HttpRequest, pool: web::Data<Pool>) -> Result<HttpResponse> {
    ...
    let safe_user_info = UserOutput {
        email: user.email,
        group_id: user.group_id,
        user_role: user.user_role,
    };
    Ok(HttpResponse::Ok().json(ResponseBody::new(constants::USER_ID, safe_user_info)))
}
